                        ...
                        <div class="single-team">
                            <div class="team-image">
                                @if (isset($user->profile))
                                    @if (file_exists(storage_path() . '/app/public/profile/' . $user->profile->profile_pic) &&
                                        !is_null($user->profile->profile_pic))
                            ...
                            <div class="team-content">
                                ...
                                <p>
                                    Organization:
                                    {{ $user->profile ? $user->profile->organization : 'No Organization' }} <br>

                                    @forelse ($user->relationalinfo as $reative)
                                        @if ($reative->relation == 'Husband')
                                            Spouse : {{ $reative->name ?? '' }}
                                        @elseif($reative->relation == 'Spouse')
                                            Spouse : {{ $reative->name ?? '' }}
                                        @endif
                                    @empty
                                    @endforelse
                                    ...
                                    Email: {{ $user->email ?? '' }}<br>
                                    Mobile No: {{ $user->profile ? $user->profile->mobile_no : '' }}<br>
                                    Blood Group: {{ $user->profile ? $user->profile->blood_group : '' }}
                                </p>
                            </div>
                        </div>
                    </div>
