    ...
    <div class="card my-4">
        <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
            <div class="bg-gradient-primary shadow-primary border-radius-lg pt-4 pb-3">
                <h6 class="text-white text-capitalize ps-3">Reunion Report</h6>
            </div>
        </div>
                    ...
                    <thead>
                        <tr>
                            <th colspan="8" style="padding-left: 75%">
                                <a class="btn btn-warning" href="{{ route('export-reunionreports') }}">Export
                                            Payment Info for Reunion</a>
                            </th>
                        </tr>
                        <tr>
                        ...
                        @foreach ($payments as $payment)
                            @if ($payment->payment_status == 'reunion_registration')
                                <tr>
                                      ...
                                      class="avatar avatar-sm me-3 border-radius-lg" alt="user1"> --}}
                                                @if ($payment->user->profile->profile_pic)
                                                    <img class="avatar avatar-sm me-3 border-radius-lg"
                                                        src="{{ asset('storage/profile/' . $payment->user->profile->profile_pic) }}"
                                                        alt="Prifle Picture">
                                                @else
                                                    <img class="avatar avatar-sm me-3 border-radius-lg"
                                    ...
                                    <td class="align-middle text-center">
                                        <span
                                            class="text-secondary text-xs font-weight-bold">{{ $payment->deposit_total_amount ?? 0 }}</span>
                                    </td>
                                    <td class="align-middle text-center">
                                        <span
                                            class="text-secondary text-xs font-weight-bold">{{ $payment->grand_total_amount ?? 0 }}</span>
                                    </td>
                                    <td class="align-middle text-center">
