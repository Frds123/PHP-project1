            ...
            <div class="card-header pb-0">
                <div class="row">
                    ...
                    <div class="col-lg-6 col-7 d-flex justify-content-end">
                        <a href="{{ route('notices.create') }}"><button class="btn btn-success">Create</button></a>
                    </div>
                </div>
            </div>
            <br>
                            ...
                            <tr>
                                ...
                                <td class="align-middle text-center">
                                    <p class="text-xs font-weight-bold mb-0">
                                        {!! Str::words(  $notice->title ?? '', 10, ' ...') !!}
                                        {{-- {{ $notice->title}} --}}
                                    </p>
                                </td>
                                <td class="align-middle text-center  d-flex justify-content-around">
                                    <a href="{{ route('notices.edit', $notice->id) }}">
                                        <span class="btn btn-sm btn-warning btn-sm">Edit</span>
                                    </a>

                                    <form class="ml-2" action="{{ route('notices.destroy', $notice->id) }}" method="post">
                                        @csrf
                                        @method('delete')
